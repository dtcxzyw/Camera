...
auto calcVertex(CommandBuffer& buffer, const DataPtr<Vert>& vert,
    ...
    buffer.runKernelLinear(runVS<Vert, Out, Uniform,Converter,vs>, vert.size(),
...
template<typename Index, typename Out, typename Uniform, typename FrameBuffer,
    ...
    auto clipedVert = buffer.allocBuffer<VertexInfo<Out>>(index.size() * 3);
    buffer.runKernelLinear(clipTriangles<Index, Out>, index.size(), triNum, vert, index, clipedVert,
